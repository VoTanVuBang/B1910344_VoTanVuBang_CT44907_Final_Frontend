<script>
    ...
    export default{
        ...
        created:async function() {
            ...
            this.bookDetail = this.book.detail;

            //Tác giả
            this.author_list =await axios.get("http://localhost:6969/api/authors/");
            this.author_list =  this.author_list.data;
            //Lấy tác giả của quyển sách được chọn
            this.selected = this.book.id_author;
        },
        methods: {
            submitBook(){
                alert("Sửa thành công");
                axios.put("http://localhost:6969/api/books/"+this.id,{
                    "name": this.bookName,
...
<template>
    ...
    >
    <div class="title text-center bg-primary">SỬA SẢN PHẨM</div>
